...
namespace QueueMetafunctions
{
     ...
     */
    std::string create_error(int errvalue)
    {
        char buffer[192] = {0};
 ...
 * @tparam MessageBufferSize Max size of the buffer.
...
template <typename T = std::void_t<>,
          ...
          int MessageBufferSize = METAQUEUE_DEFAULT_MAX_MESSAGE_SIZE,
          int QueueFlags = METAQUEUE_DEFAULT_QUEUE_FLAGS>
class metaqueue
{
    using type = metaqueue<T, QueuePermission, MaxMessages, MaxMessageSize, MessageBufferSize, QueueFlags>; //> Current meta type.
...
private:
    ...
    char buffer[MessageBufferSize]; //> Raw buffer where the bytes will be stored while doing queue.
    ...
    void clean_buffer()
    {
        std::memset(buffer, 0, MessageBufferSize);
    }
...
public:
    ...
    value_type pop(int timeout = -1, unsigned int priority = 0)
    {
        try
        {
            ...
            auto response = QueueMetafunctions::pop<value_type, is_memcpyed, is_trivial>::run(queue_fd, buffer, MessageBufferSize, timeout, &priority);
